export function setupSoundProtection() {
  ...
  OriginalSound.prototype.play = async function (...args) {
    ...
    const name = match?.name || game.i18n.localize("k4lt-assets.SoundProtection.Unknown");
    kultLogger(`🎵 Playing : ${name} | 📂 ${path}`);

    if (protectedPaths.has(path)) {
      kultLogger(`⏳ Countdown launched for track : ${name}`);
      ui.notifications.warn(game.i18n.localize("k4lt-assets.SoundProtection.RestrictedNotice"));

      ChatMessage.create({
        user: game.user.id,
        speaker: { alias: "Sound Tracker" },
        content: game.i18n.format("k4lt-assets.SoundProtection.RestrictedChat", { name }),
      ...
      setTimeout(() => {
        ...
        ChatMessage.create({
          user: game.user.id,
          speaker: { alias: "Sound Tracker" },
          content: game.i18n.format("k4lt-assets.SoundProtection.RestrictedEnd", { name }),
          whisper: ChatMessage.getWhisperRecipients("GM")
        });
