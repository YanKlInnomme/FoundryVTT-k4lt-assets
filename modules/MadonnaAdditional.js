const PATHS = {
    ...
    new: "modules/k4lt-assets/img/the-black-madonna/portraits/"
};
...
export async function checkForTheBlackMadonnaModule() {
    ...
    const hideDialog = game.settings.get("k4lt-assets", "hideAssetsDialog");
    if (hideDialog) {
        kultLogger("The dialog box is disabled via the settings.");
        const useContent = game.settings.get("k4lt-assets", "useAdditionalContent");
        return useContent ? applyAdditionalContent() : removeAdditionalContent();
    }

    const confirmed = await new Promise((resolve) => {
        const content = `
            <p>${game.i18n.localize("k4lt-assets.Dialog.moduleActivatedContent")}</p>
            <p>${game.i18n.localize("k4lt-assets.Dialog.moduleActivatedQuestion")}</p>
            <div style="display: flex; align-items: center; justify-content: center; margin-top: 10px;">
                <label style="display: flex; align-items: center;">
                    <input type="checkbox" id="dontShowAgain" style="margin-right: 5px;" />
                    ${game.i18n.localize("k4lt-assets.Dialog.doNotShowAgain")}
                </label>
            </div>
        `;

        new Dialog({
            title: game.i18n.localize("k4lt-assets.Dialog.moduleActivatedTitle"),
            content,
            buttons: {
                yes: {
                    ...
                    callback: (html) => {
                        if (html.find("#dontShowAgain").is(":checked")) {
                            game.settings.set("k4lt-assets", "hideAssetsDialog", true);
                        }
                        resolve(true);
                    }
                },
    ...
    await game.settings.set("k4lt-assets", "useAdditionalContent", confirmed);
    return confirmed ? applyAdditionalContent() : removeAdditionalContent();
}
...
async function applyAdditionalContent() {
    ...
    for (let actor of npcs) {
        ...
        if (newImg && newImg !== currentImg) {
            const existingFlag = actor.getFlag("k4lt-assets", "originalPortrait");
            if (!existingFlag) {
                await actor.setFlag("k4lt-assets", "originalPortrait", currentImg);
            }
...
async function removeAdditionalContent() {
    kultLogger("❌ Removing additional content from journal entries.");
    const npcs = game.actors.filter(actor => actor.type === "npc");

    for (let actor of npcs) {
        const originalImg = actor.getFlag("k4lt-assets", "originalPortrait");
        if (originalImg && actor.img !== originalImg) {
            ...
            await actor.unsetFlag("k4lt-assets", "originalPortrait");
        }
...
async function updateOrRestoreTokens(fromPath, toPath, isRestoring = false) {
    for (let scene of game.scenes) {
        ...
        for (let tokenData of scene.tokens) {
            ...
            if (isRestoring) {
                const saved = await token.getFlag("k4lt-assets", "originalTokenImage");
                if (saved && currentImg !== saved) {
                    targetImg = saved;
                    await token.unsetFlag("k4lt-assets", "originalTokenImage");
                ...
                const existingFlag = await token.getFlag("k4lt-assets", "originalTokenImage");
                if (!existingFlag && targetImg) {
                    await token.setFlag("k4lt-assets", "originalTokenImage", currentImg);
                }
            }
...
async function updateOrRestorePrototypeTokens(actor, fromPath, toPath, isRestoring = false) {
    ...
    if (isRestoring) {
        const originalProto = actor.getFlag("k4lt-assets", "originalPrototypeImage");
        if (originalProto && protoImg !== originalProto) {
            ...
            await actor.unsetFlag("k4lt-assets", "originalPrototypeImage");
        }
    } else {
        const newProtoImg = getUpdatedImage(actor, protoImg, fromPath, toPath);
        if (newProtoImg && newProtoImg !== protoImg) {
            const existingFlag = actor.getFlag("k4lt-assets", "originalPrototypeImage");
            if (!existingFlag) {
                await actor.setFlag("k4lt-assets", "originalPrototypeImage", protoImg);
            }
...
export function handleRenderJournalEntrySheet(app, html, data) {
    ...
    wait(100).then(() => {
        const useAdditionalContent = game.settings.get("k4lt-assets", "useAdditionalContent");
        kultLogger("Setting 'useAdditionalContent':", useAdditionalContent);
        const isGM = game.user.isGM;
        kultLogger("User is GM:", isGM);

        const conditions = {
            showIfAITrue: useAdditionalContent,
            showIfAdmin: isGM,
        };

        const conditionalElements = html.querySelectorAll(".journal-entry-content .conditional");
        ...
        conditionalElements.forEach((element, index) => {
            ...
            kultLogger(`Element #${index} (${conditionKey}): ${shouldShow ? "shown" : "hidden"}`);
...
async function importAdditionalScenes() {
    const pack = game.packs.get('k4lt-assets.additional-scenes--tbm');
    if (!pack) {
        kultLogger("Compendium k4lt-assets.additional-scenes--tbm not found.");
        return;
    }
    await pack.getIndex();
...
async function importAdditionalPlaylists() {
    const pack = game.packs.get('k4lt-assets.additional-playlists--tbm');
    if (!pack) {
        kultLogger("Compendium k4lt-assets.additional-playlists--tbm not found.");
        return;
    }
...
async function removeAdditionalScenes() {
    const scenesToDelete = game.scenes.filter(scene => 
        scene.flags?.["k4lt-assets"]?.sourceCompendium === 'k4lt-assets.additional-scenes--tbm'
    );
